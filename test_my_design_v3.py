...
# transaction--generater和driver之间传送的数据
class chnl_trans:
...
# 验证环境的单位
class chnl_agent:
    def __init__(self, ch_id, chnl_monitor_name="chnl_monitor", name = "chnl_agent"):
        self.name = name
        self.driver = driver(ch_id)
        self.monitor = chnl_monitor(ch_id, chnl_monitor_name)
    
    async def run(self, dut):    
        await cocotb.start(self.driver.run(dut))
        await cocotb.start(self.monitor.run(dut))
...
class chnl_root_test:
    def __init__(self, name = "chnl_root_test"):
        self.name = name
        self.agents = [chnl_agent(0, "chnl_monitor0", "chnl_agent0"), 
                        chnl_agent(1, "chnl_monitor1", "chnl_agent1"), 
                         chnl_agent(2, "chnl_monitor2", "chnl_agent2")]        
    ...
    async def run(self, dut): 
        for agent in self.agents:
            await cocotb.start(agent.run(dut))
